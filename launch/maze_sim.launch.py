...
import xacro # تأكدي من استيرادها
...
def generate_launch_description():
    ...
    # 3. معالجة ملف Xacro باستخدام المكتبة الرسمية
    ...
    # 4. إحضار الروبوت (Spawn)
    spawn_robot = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=[
            '-name', 'turtlebot4',
            '-string', robot_description_content, # استخدام الكود المباشر أفضل من التوبيك أحياناً
            '-x', '0.0', '-y', '0.0', '-z', '0.1'
        ],
        output='screen',
    )

    # 5. الجسر (Bridge) - تأكدي من المجلد config أو parameters حسب setup.py
